...
export default function ClassEdit() {
  ...
  const { getClassById, updateClass } = useData();
  const { addToast } = useToast();
  const cls = getClassById(id!);

  const [form, setForm] = useState<FormData>(() => {
    if (!cls) return { name: "", grade: "", section: "", teacherName: "", studentCount: "", room: "", schedule: "" };
    return {
      name: cls.name,
      grade: cls.grade,
      section: cls.section,
  ...
  if (!cls) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <p className="text-lg font-medium text-gray-600">Class not found</p>
        <button onClick={() => navigate("/classes")} className="mt-4 text-sm text-primary-600 hover:underline">
          Back to Classes
        </button>
      </div>
    );
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    updateClass(id!, { ...form, studentCount: Number(form.studentCount) });
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-3">
            ...
            <h2 className="text-sm font-semibold text-gray-800">Class Information</h2>
          </div>
          ...
          <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                ...
                <input type="text" name="name" value={form.name} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Grade 10 - A" className={inputCls("name")} />
              </div>
          ...
          <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" name="teacherName" value={form.teacherName} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Dr. Sunita Sharma" className={inputCls("teacherName")} />
              </div>
              {errorMsg("teacherName") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("teacherName")}</p>}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <Users className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <input type="number" name="studentCount" value={form.studentCount} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. 35" className={inputCls("studentCount")} />
              </div>
              {errorMsg("studentCount") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("studentCount")}</p>}
            </div>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                ...
                <input type="text" name="room" value={form.room} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Room 101" className={inputCls("room")} />
              </div>
              {errorMsg("room") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("room")}</p>}
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              ...
              <div className="relative">
                ...
                <input type="text" name="schedule" value={form.schedule} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Mon-Fri, 8:00-2:30" className={inputCls("schedule")} />
              </div>
              {errorMsg("schedule") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("schedule")}</p>}
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => navigate("/classes")} className="rounded-lg border border-gray-200 px-5 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50">Cancel</button>
