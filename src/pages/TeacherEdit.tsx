...
export default function TeacherEdit() {
  ...
  const { getTeacherById, updateTeacher } = useData();
  const { addToast } = useToast();
  const teacher = getTeacherById(id!);

  const [form, setForm] = useState<FormData>(() => {
    if (!teacher) return { name: "", email: "", phone: "", subject: "", department: "", qualification: "", joinDate: "", status: "active" };
    return {
      name: teacher.name,
      email: teacher.email,
      phone: teacher.phone,
  ...
  if (!teacher) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        <p className="text-lg font-medium text-gray-600">Teacher not found</p>
        <button onClick={() => navigate("/teachers")} className="mt-4 text-sm text-primary-600 hover:underline">
          Back to Teachers
        </button>
      </div>
    );
      ...
      <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" name="name" value={form.name} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Sita Sharma" className={inputCls("name")} />
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" name="email" value={form.email} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. sita@school.edu" className={inputCls("email")} />
              </div>
        ...
        <div className="mt-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
            {/* Subject */}
            <div>
              <label className="mb-1.5 flex items-center gap-1 text-sm font-medium text-gray-700">Subject <span className="text-danger-500">*</span></label>
              <div className="relative">
                <BookOpen className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <input type="text" name="subject" value={form.subject} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. Mathematics" className={inputCls("subject")} />
              </div>
              {errorMsg("subject") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("subject")}</p>}
            </div>
            {/* Department */}
            <div>
              <label className="mb-1.5 flex items-center gap-1 text-sm font-medium text-gray-700">Department <span className="text-danger-500">*</span></label>
              <div className="relative">
                <Building2 className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <select name="department" value={form.department} onChange={handleChange} onBlur={handleBlur} className={selectCls("department")}>
                  <option value="" disabled>Select department</option>
                  {departments.map((d) => <option key={d} value={d}>{d}</option>)}
                </select>
              </div>
              {errorMsg("department") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("department")}</p>}
            </div>
            {/* Qualification */}
            <div>
              <label className="mb-1.5 flex items-center gap-1 text-sm font-medium text-gray-700">Qualification <span className="text-danger-500">*</span></label>
              <div className="relative">
                <GraduationCap className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <input type="text" name="qualification" value={form.qualification} onChange={handleChange} onBlur={handleBlur} placeholder="e.g. M.Sc. Mathematics" className={inputCls("qualification")} />
              </div>
              {errorMsg("qualification") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("qualification")}</p>}
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
          <div className="mb-5 flex items-center gap-3">
            ...
            <h2 className="text-sm font-semibold text-gray-800">Employment</h2>
          </div>
          <div className="grid gap-x-4 gap-y-5 sm:grid-cols-2">
            {/* Join Date */}
            <div>
              <label className="mb-1.5 flex items-center gap-1 text-sm font-medium text-gray-700">Join Date <span className="text-danger-500">*</span></label>
              <div className="relative">
                <CalendarDays className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
                <input type="date" name="joinDate" value={form.joinDate} onChange={handleChange} onBlur={handleBlur} className={inputCls("joinDate")} />
              </div>
              {errorMsg("joinDate") && <p className="mt-1 flex items-center gap-1 text-xs text-danger-500"><AlertCircle className="h-3 w-3 shrink-0" />{errorMsg("joinDate")}</p>}
            </div>
            {/* Status */}
            <div>
        ...
        <div className="mt-6 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => navigate("/teachers")} className="rounded-lg border border-gray-200 px-5 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50">Cancel</button>
